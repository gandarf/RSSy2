...
- **비동기 처리 및 진행 상황 표시:** RSS 피드 가져오기 및 AI 요약 과정은 시간이 오래 걸릴 수 있으므로, **비동기(Asynchronous)** 방식으로 처리하여 UI 멈춤 현상을 방지합니다. 사용자는 웹 UI를 통해 실시간으로 작업의 진행 상황(예: "피드 가져오는 중 3/5", "AI 요약 중 1/10")을 확인할 수 있습니다.
...
- **데이터 저장 모듈:** 피드, 기사, 요약 및 **작업 상태(Job Status)**를 **SQLite 데이터베이스**에 저장하고 관리합니다. 동시성 제어를 고려하여 트랜잭션을 관리합니다.
- **표시 모듈 (UI):** 요약된 보고서를 렌더링하고, **AJAX/Polling**을 통해 백엔드에서 작업 진행 상황을 실시간으로 가져와 보여줍니다.
